...
until ( $epoch > ( $^T + ( 86400 * 100 ) ) ) {
    ...
    dump_epoch($epoch, '1234567') if $epoch =~ m/1234567/;

    # lots of the same digit
    dump_epoch($epoch, 'repetition') if $epoch =~ m/([0-9])\1\1\1\1\1\1+/;

    # repeating doublets.
    dump_epoch($epoch, 'doublets') if $epoch =~ m/([0-9]{2})\1\1\1/;
    ...
    # vertical symmetry, kinda boring, happens too often.
